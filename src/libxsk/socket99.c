...
int socket99_snprintf(char* buf, size_t buf_size, const socket99_result* res) {
    ...
    return snprintf(
        buf, buf_size, "%s: %s", status_key(res->status),
        (res->status == SOCKET99_ERROR_GETADDRINFO ? gai_strerror(res->getaddrinfo_error) : strerror(res->saved_errno)));
}
...
static bool set_socket_options(const socket99_config* cfg, socket99_result* out, int fd) {
    for (int i = 0; i < SOCKET99_MAX_SOCK_OPTS; i++) {
        ...
        if (setsockopt(fd, SOL_SOCKET, opt->option_id, opt->value, opt->value_len) < 0) {
