...
// Pointer to the XDP program used by the AF_XDP socket.
static struct xdp_program* prog;

/**
 * Structure that holds information about a user space memory region (umem).
 * It contains a producer ring (fq), a consumer ring (cq), a pointer to the umem itself, and a pointer to the buffer.
 */
...
static uint64_t xsk_alloc_umem_frame(struct xsk_socket_info* xsk) {
    ...
    frame = xsk->umem_frame_addr[--xsk->umem_frame_free];  // Allocate starting
                                                           // from the end
...
/**
 * This function completes the transmission of packets that have been sent by the user space program.
 * It sends a message to the AF_XDP socket to notify the kernel that there are packets to be processed.
 * It then collects and frees any completed TX buffers and updates the number of outstanding TX buffers.
 *
 * @param xsk A pointer to the xsk_socket_info struct that contains information about the AF_XDP socket.
 */
static void complete_tx(struct xsk_socket_info* xsk) {
    ...
    sendto(xsk_socket__fd(xsk->xsk), NULL, 0, MSG_DONTWAIT, NULL, 0);  // Notify the kernel that there are packets to be processed
    ...
    if (completed > 0) {
        ...
        xsk_ring_cons__release(&xsk->umem->cq, completed);  // Release the completed TX buffers
...
static bool process_packet(struct xsk_socket_info* xsk, uint64_t addr, uint32_t len) {
    ...
    if (true) {
        ...
        // Check if packet is IPv6 ICMP ECHO REQUEST
        ...
        xsk_ring_prod__tx_desc(&xsk->tx, tx_idx)->addr = addr;  // We send the same buffer back
        xsk_ring_prod__tx_desc(&xsk->tx, tx_idx)->len = len;    // Same length
        xsk_ring_prod__submit(&xsk->tx, 1);                     // Submit the packet
        xsk->outstanding_tx++;  // Increase outstanding TX counter (outstanding TX means TX that have been submitted but not
                                // completed yet)
...
/**
 * This function handles the received packets from the AF_XDP socket.
 * It peeks into the RX ring buffer to check if there are any packets received.
 * If there are packets received, it reserves free frames from the UMEM's FQ ring buffer
 * and fills them with the received packets. It then submits the filled frames to the FQ ring buffer.
 * It then processes the received packets and updates the statistics.
 * Finally, it releases the RX ring buffer and checks if the kernel needs to be woken up for transmission.
 *
 * @param xsk A pointer to the xsk_socket_info struct containing the AF_XDP socket information.
 */
static void handle_receive_packets(struct xsk_socket_info* xsk) {
    ...
    for (i = 0; i < rcvd; i++) {
        ...
        // If we can't process the packet, free the frame
...
/**
 * This function receives and processes packets from an AF_XDP socket.
 * It uses a poll loop to wait for incoming packets and calls the
 * handle_receive_packets function to process them.
 *
 * @param cfg A pointer to the configuration struct.
 * @param xsk_socket A pointer to the xsk_socket_info struct.
 */
static void rx_and_process(struct config* cfg, struct xsk_socket_info* xsk_socket) {
    ...
    while (!global_exit) {
        if (cfg->xsk_poll_mode) {
            ret = poll(fds, nfds, -1);  // Wait indefinitely for packets to arrive
            if (ret <= 0 || ret > 1) {  // Error or no packets received
                continue;               // Try again
            }
        }
        sleep(10);
